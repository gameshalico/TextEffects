    ...
    public static class TagParser
    {
        ...
        public static (string Text, TagInfo[] Tags) Parse(string input, bool unescapeXml = false)
        {
            ...
            foreach (Match match in TagRegex.Matches(input))
            {
                ...
                if (unescapeXml)
                {
                    textSegment = XmlEscapeUtility.UnescapeXml(textSegment);
            ...
            if (unescapeXml)
            {
                finalSegment = XmlEscapeUtility.UnescapeXml(finalSegment);
        ...
        /// <summary>
        /// テキストからカスタムタグ(TMPタグ以外)を除去します。TMPタグは残します。
        /// getPreferredValuesで幅計算する際に使用します。
        /// </summary>
        /// <param name="text">処理対象のテキスト</param>
        /// <returns>カスタムタグを除去したテキスト(TMPタグは残る)</returns>
        public static string RemoveCustomTags(string text)
        {
            if (string.IsNullOrEmpty(text))
                return text;

            var result = TagRegex.Replace(text, match =>
            {
                var tagName = match.Groups["TagName"].Value;
        ...
        /// <summary>
        /// テキストからすべてのタグとゼロ幅スペースを除去し、実際の表示文字のみを取得します。
        /// 文字数カウントに使用します。
        /// </summary>
        /// <param name="text">処理対象のテキスト</param>
        /// <returns>プレーンテキスト(表示される文字のみ)</returns>
        public static string GetPlainText(string text)
        {
            ...
            // すべてのタグを除去(TMPタグも含む)
            var result = TagRegex.Replace(text, string.Empty);
